...
<template>
  <div class="content">
    ...
    <div class="list">
      <div class="table">
        <el-table v-loading="loading" ref="breakTableRef" :data="tableData" tooltip-effect="dark" style="width: 100%"
          :height="height" border :key="Math.random()" default-expand-all row-key="realTradeId"
          :row-class-name="tableRowClassName" :cell-style="cellStyleUpdate"
          :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
          header-row-style="height:30px;line-height:30px;" header-cell-style="background:#f8f8f8;"
          ...
          <el-table-column fixed="right" align="center" label="操作" width="200px">
            <template slot-scope="scope">
              <el-popover v-if="setAuth('break:back') &&
          [3, 4].indexOf(scope.row.jiaogeStatus) !== -1 &&
          [22].indexOf(scope.row.status) === -1
          " placement="bottom-end" :ref="`popover-breakback-${scope.$index}`">
                <p>
          ...
          ">取消</el-button>
                  <el-button type="text" @click="handleBreakBackClick(scope)">确认</el-button>
                </div>
                <el-button type="text" slot="reference" class="ml10">改交割</el-button>
              </el-popover>
              <el-popover v-if="setAuth('break:upgrade') &&
          [3, 5].indexOf(scope.row.jiaogeStatus) !== -1 &&
          [22].indexOf(scope.row.status) === -1
          " placement="bottom-end" :ref="`popover-breakupgrade-${scope.$index}`">
                <p>
                  确认"{{ scope.row.tscode }}"<span class="color-red">
                    {{
        ...
        handlePopoverClose(
          ...
          ">取消</el-button>
                  <el-button type="text" @click="handleBreakUpgradeClick(scope)">确认</el-button>
                </div>
          ...
          scope.row.jiaogeStatus === 3
            ...
            : scope.row.jiaogeStatus === 5
              ...
              <el-button v-if="[3].indexOf(scope.row.jiaogeStatus) !== -1 &&
          setAuth('break:redo') &&
          [22].indexOf(scope.row.status) === -1
          " type="text" @click="handleOpenBreakEditDialog(1, scope.row)" class="ml10">续作</el-button>
              <!-- <el-button
                v-if="
                ...
                >增改</el-button
              > -->
              <el-button v-if="[2, 3, 4].indexOf(scope.row.jiaogeStatus) !== -1 &&
          [22].indexOf(scope.row.status) === -1 &&
          setAuth('break:addback') &&
          (['研究员'].indexOf(userInfo.roleName) === -1 ||
            (['研究员'].indexOf(userInfo.roleName) !== -1 &&
              [3].indexOf(scope.row.jiaogeStatus) === -1))
          " type="text" @click="handleOpenBreakEditDialog(3, scope.row)" class="ml10">补单</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    </div>
    <el-dialog :title="currentType === 1 ? '违约续作' : currentType === 3 ? '补单' : ''" :width="returnFrameW(600) + 'px'"
      :visible.sync="dialogBreakEditFormVisible" append-to-body :destroy-on-close="true" :close-on-click-modal="false">
      <break-edit :row="currentRow" :type="currentType" @change="handleBreakEditDialogVisible"></break-edit>
    </el-dialog>
  </div>
</template>
