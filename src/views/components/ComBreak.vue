...
export default {
  ...
  methods: {
    ...
    cellStyleUpdate(row, column, rowIndex, columnIndex) {
      if (
        row.column.label === "违约量" &&
        parseInt(row.row.volume) > row.row.weiyueAmount
      ) {
        return "color:orange";
      }
    },
    ...
    handleBreakBackClick: debounce(function(scope) {
      api
        .dealBreakReturn({ realTradeId: scope.row.realTradeId })
        .then(response => {
          if (response && response.code === "00000") {
            this.$message({
              message: "操作成功",
    ...
    handleBreakUpgradeClick: debounce(function(scope) {
      api
        .dealBreakUpgrade({ realTradeId: scope.row.realTradeId })
        .then(response => {
          if (response && response.code === "00000") {
            this.$message({
              message: "操作成功",
