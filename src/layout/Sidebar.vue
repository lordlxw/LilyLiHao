<template>
  <div>
    <el-menu :default-active="this.$route.path" background-color="#202020" text-color="#fff" active-text-color="#fff"
      router :default-openeds="defaultOpeneds" v-if="userInfo">
      <template v-for="(item, index) in userInfo.menutree">
        <el-submenu v-if="item.children && item.children.length > 0" :index="index + 1 + ''" :key="index">
          <template slot="title">
            <i :class="item.icon" class="mr10 menu-icon"></i>
            <span class="menu-tit">{{ item.menuName }}</span>
          </template>
          <el-menu-item v-for="item2 in item.children" :route="item2.component" :index="item2.component"
            :key="item2.component" @click="
              handleNavigator(
        ...
        <el-menu-item v-else :index="item.component === null ? index + 1 + '' : item.component" :route="item.component"
          :key="item.component" @click="
            handleNavigator(
              [`${item.menuName}`],
              [item.menuId],
              `${item.component}`
