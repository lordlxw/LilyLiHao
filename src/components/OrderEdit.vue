  ...
  <div>
    ...
    <div class="mt30 steps-desc">
      <el-descriptions title="订单信息" direction="vertical" size="medium" :column="5" border>
        <template slot="extra" v-if="setAuth('system:order:edit')">
          ...
          <div v-else>
            ...
            <el-button v-if="viewWork == 0" type="primary" size="small" @click="operateTrade">操作询价单</el-button>
            <el-button v-if="viewWork == 1" type="primary" size="small"
              @click="handleInquiryDealConfirmClick">同意修改</el-button>
            <el-button v-if="viewWork == 1" type="primary" size="small"
              @click="viewWork = 0, dataForm.realTrades = [], dataForm.target = null">取消操作</el-button>
          </div>
        </template>
        <el-descriptions-item label="研究员">{{ this.userSummary && this.userSummary.length > 0 ?
          this.userSummary.find(n => n.userId === currentRow.createBy).nickName : '--' }}</el-descriptions-item>
      ...
      <el-descriptions v-if="![4, 6, 3].includes(dataForm.target) && currentRow.children.length > 0"
        v-for="(chid, index) in currentRow.children" :key="chid.id" direction="vertical" size="medium" :column="5"
        border>
        <el-descriptions-item :label="'子成交单' + index">{{ chid.tradeNum }}</el-descriptions-item>
        <el-descriptions-item label="成交价">{{ chid.realPrice }}</el-descriptions-item>
      ...
      <el-descriptions v-if="[4, 6, 3].includes(dataForm.target) && viewWork == 1 && dataForm.realTrades.length > 0"
        v-for="(chid, index) in dataForm.realTrades" :key="index" direction="vertical" size="medium" :column="5" border>
        ...
        <el-descriptions-item label="成交量">
          <el-input-number class="ipt-volume" v-model="chid.turnover" :step="1000" :min="1000"
            :max="parseInt(currentRow.volume)" step-strictly></el-input-number>
        </el-descriptions-item>
        <el-descriptions-item label="成交时间">{{ chid.createTime || '0000-00-00 00:00:00' }}</el-descriptions-item>
        <el-descriptions-item label="操作">
          <el-button v-if="viewWork == 1 && currentRow.lock" type="primary" size="small"
            @click="changeRealTrade(index)">移除</el-button>
        </el-descriptions-item>
      </el-descriptions>
      <el-descriptions v-if="[4, 6, 3].includes(dataForm.target) && viewWork == 1 && currentRow.lock"
        direction="vertical" size="medium" :column="5" border>
        <el-descriptions-item labelStyle="display:none">
          <el-card shadow="hover" class="text-center" @click.native="changeRealTrade(-1)">
