<template>
    <div class=" order-notifiy drag-container">
        <transition-group name="slide-right" tag="div" v-if="messages.length > 0">
            <div class="notification draggable" draggable="true" @dragstart="dragStart"
                v-for="(item, index) in messages" :key="item.id" @click="viewChat(item)" :target-index="index">
                <i class="el-notification__icon el-icon-warning"></i>
                <div class="el-notification__group is-with-icon">
                    <div class="el-notification__title">{{ item.company }}-{{ item.name }}-通知消息</div>
                    <div class="el-notification__content">
                        <p>{{ item.company }}-{{ item.name }} : {{ item.chatMessage }}</p>
                    </div>

                    <div class="el-notification__closeBtn">
                        <div class="el-icon-close icon" @click.stop="updateChatMessage(item)"></div>
                    </div>
                </div>
            </div>
        </transition-group>
    ...
    methods: {
        ...
        updateChatMessage(element) {
            Promise.all([
                ...
                const { code } = await apiTrade.updateChatMessages({ chatId }, element.id)
                if (code === '00000') {
        ...
        initChatMessages(date) {
            Promise.all([
                ...
                const { value: msgs } = await apiTrade.getSysChatMessagesByCondition({ createTime, chatId: -1 })
        ...
        dragStart(event) {
            ...
            event.dataTransfer.setData('text/plain', JSON.stringify(this.messages[index]));
        },
    },
    computed: {
        ...mapGetters({
